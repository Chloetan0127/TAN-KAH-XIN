...
if(isset($_GET["Loginbtn"]))
{
	...
	if($name && $password)
	{
		...
		//Check Admin
		$resultA = mysqli_query($connect,"select * from admin where adminName = '$name' and adminPass = '$password'");
		$rowA = mysqli_fetch_array($resultA,MYSQLI_ASSOC);
		

		if(mysqli_num_rows($resultU) == 1)
		{
			$_SESSION["id"]=$rowU["cusId"];
...
if(isset($_POST["Signupbtn"]))
{
	...
	if($password==$cpassword)
	{
		...
		else
		{
			...
			?><script>alert("Your registration is completed..");</script><?php
		}
